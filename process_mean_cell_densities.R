...
# CHRYSAORA data processing -----------------------------------------------

  #c <- c[complete.cases(c),] #remove the 3 cases of NAs

  chrys <- c[,c(4:length((c)))]
  chrys <- plyr::rename(c, c("JOIN_FID" = "cell_id"))
  ...
  chrys_m <- merge(chrys, a ,by = "cell_id")
  chrys_m <- plyr::rename(chrys_m, c("shape_area" = "area_m2"))

  #calculate annual mean biomass & population density in each fish net cell | STEP 2 in JMP workflow

  yr_cell_stats_chrys <- ddply(chrys_m, .variables = c("cell_id","year"), function(x){
    ...
    if(nrow(x)>0){
      ...
      if(sum(x$biomass_den_kg_m3)>0){
        ...
        chrys_y <- data.frame(cell_id, decslat_cell_ctr, decslon_cell_ctr, area_m2, year, agg_grp, subregion_depth, subregion_alongshore, bio_mean_kg, bio_var_kg, bio_CI95_kg, total_bio_wwt_kg_mean, total_bio_wwt_kg_var,
                        pop_mean_no_m3, pop_var_no_m3, pop_CI95_no_m3, total_indiv_mean, total_indiv_var, n_obs)
  ...
  yr_cell_stats_chrys$agg_grp <- as.character(yr_cell_stats_chrys$agg_grp)


  # Whole Gulf of Mexico time-series CHRYS ------
  yr_stats_chrys <- ddply(yr_cell_stats_chrys, .variables = c("year"), function(x){
    ...
    if(nrow(x)>0){
      ...
      if(sum(x$total_bio_wwt_kg_mea)>0){
        ...
        chrys_y <- data.frame(year, agg_grp, bio_wt_mean_kg, bio_delta_mean_kg, bio_delta_var_kg, bio_delta_sd_kg,
                        pop_wt_mean_no_m3, pop_delta_mean_no_m3, pop_delta_var_no_m3, pop_delta_sd_no_m3, n_obs)
  ...
  #save(yr_stats, file = "Aurelia_SEAMAP_All_GoM_time-series.Rdata")
  # By Subregion_alongshore & depth time-series ------
  yr_shore_depth_stats_chrys <- ddply(yr_cell_stats_chrys, .variables = c("year","subregion_depth","subregion_alongshore"), function(x){
    ...
    if(nrow(x)>0){
      ...
      if(sum(x$total_bio_wwt_kg_mea)>0){
        ...
        chrys_y <- data.frame(agg_grp, bio_wt_mean_kg, bio_delta_mean_kg, bio_delta_var_kg, bio_delta_sd_kg,
                        pop_wt_mean_no_m3, pop_delta_mean_no_m3, pop_delta_var_no_m3, pop_delta_sd_no_m3, n_obs)
  ...
  # TX - INSHORE
  chrys.tx.insh <- yr_shore_depth_stats_chrys[yr_shore_depth_stats_chrys$subregion_depth == "1_inshore" & yr_shore_depth_stats_chrys$subregion_alongshore == "1_Tex",]
 ...
 # p <- plot(x=tx.insh$year, y=log(tx.insh$pop_wt_mean_no_m3+1), type = "b")
  ...
  # TX - SHELF
  chrys.tx.shelf <- yr_shore_depth_stats_chrys[yr_shore_depth_stats_chrys$subregion_depth == "2_shelf" & yr_shore_depth_stats_chrys$subregion_alongshore == "1_Tex",]
  ...
  # LA - INSHORE
  chrys.la.insh <- yr_shore_depth_stats_chrys[yr_shore_depth_stats_chrys$subregion_depth == "1_inshore" & yr_shore_depth_stats_chrys$subregion_alongshore == "2_Lou",]
  ...
  # LA - SHELF
  chrys.la.shelf <- yr_shore_depth_stats_chrys[yr_shore_depth_stats_chrys$subregion_depth == "2_shelf" & yr_shore_depth_stats_chrys$subregion_alongshore == "2_Lou",]
  ...
  # FL - INSHORE
  chrys.fl.insh <- yr_shore_depth_stats_chrys[yr_shore_depth_stats_chrys$subregion_depth == "1_inshore" & yr_shore_depth_stats_chrys$subregion_alongshore == "3_Fla",]
  ...
  # L - SHELF
  chrys.fl.shelf <- yr_shore_depth_stats_chrys[yr_shore_depth_stats_chrys$subregion_depth == "2_shelf" & yr_shore_depth_stats_chrys$subregion_alongshore == "3_Fla",]
  ...
  # By Subregion_alongshore time-series ------
  yr_shore_stats_chrys <- ddply(yr_cell_stats_chrys, .variables = c("year","subregion_alongshore"), function(x){
  ...
  save(yr_shore_stats_chrys, file = "Aurelia_SEAMAP_GoM_region_time-series.Rdata")


  # TEXAS
  ctx <- yr_shore_stats_chrys[yr_shore_stats_chrys$subregion_alongshore == "1_Tex",]
  ...
  # LOUISIANA
  cla <- yr_shore_stats_chrys[yr_shore_stats_chrys$subregion_alongshore == "2_Lou",]
  ...
  # FLORIDA
  cfl <- yr_shore_stats_chrys[yr_shore_stats_chrys$subregion_alongshore == "3_Fla",]
