...
#Created by: Stacy Calhoun, 31 Aug 18
#Modified by : Kelly Robinson 2024-01-18
...
#read-in support files needed to run this script -----
support.files <- list.files("filesfor_SEAMAP_QueriesRcode/", recursive = T)

  NameTrans = read_xlsx(path = 'filesfor_SEAMAP_QueriesRcode/NameTranslator_table201305.xlsx')

  #exclude unidentified fish "UNID.FISH" as they have no length to weight conversion equations
  NameTrans <- NameTrans[NameTrans$TAXONOMIC != "UNID.FISH",]
  NameTrans <- NameTrans[!is.na(NameTrans$TAXONOMIC),]

  starec_rev = suppressWarnings(read.csv(file = "filesfor_SEAMAP_QueriesRcode/STAREC_rev20190402.csv",
                                    stringsAsFactors = F, header = T, strip.white = T, ))
  starec_rev$X <- NULL

# Set specific interest groups from Name Translator Table ----

  # If you want to select data using only major group; meaning you want ALL
  # the taxa in that major or aggregrated group (see Name Translator Table file)

  #selecting data based only the major group: options --> fish; invertebrate; jellyfish
    unique(NameTrans$major_group)

    use.major.grp <- 'no'
    major.grp = "jellyfish"

  # set aggregated group
    #(see Name Translator file for possible options)
    unique(NameTrans$AggGrp_20130430)

  use.aggregrated.group <- 'no'
  aggregated.grp <- c("small flatfish")

  #identifying the major group from a specific species or multiple species
  use.taxa <- 'yes' #change to 'yes' to use the values in the object 'taxa' below
  #taxa = c("AURELIA", "AURELIA AURITA","CHRYSAORA","CHRYSAORA QUINQUECIRRHA","DACTYLOMETRA QUINQUECIRRHA")
  taxa = c("AURELIA", "AURELIA AURITA")


#identifying the species belonging to either the major group or aggregrated identified above
if(use.major.grp=='yes'){
  taxa <- NameTrans[NameTrans$major_group %in% major.grp,]$TAXONOMIC

  } else if(use.aggregrated.group == 'yes'){
      taxa <- NameTrans[NameTrans$AggGrp_20130430 %in% aggregated.group,]$TAXONOMIC

      } else if(use.taxa == 'yes'){
        aggregated.grp <- unique(NameTrans[NameTrans$TAXONOMIC %in% taxa,]$AggGrp_20130430)
      } else {
        message("Please indicate above, in lines 51-63, the species (taxa), aggregrated group,
                or major group you would like to process data for.")
      }



# load SEAMAP data files "https://seamap.gsmfc.org/datarequests/index.php" -----
INGEST_DATA = read.csv('20240118_seamap_trawl_csv/INGEST_DATA.csv')
INVREC = read.csv('20240118_seamap_trawl_csv/INVREC.csv')
ISTREC = read.csv('20240118_seamap_trawl_csv/ISTREC.csv')
NEWBIOCODESBIG = read.csv('20240118_seamap_trawl_csv/NEWBIOCODESBIG.csv')
SHRREC = read.csv('20240118_seamap_trawl_csv/SHRREC.csv')
VESSELS = read.csv('20240118_seamap_trawl_csv/VESSELS.csv')
CTDCASTREC = read.csv('20240118_seamap_trawl_csv/CTDCASTREC.csv')
CTDREC = read.csv('20240118_seamap_trawl_csv/CTDREC.csv')
GLFREC = read.csv('20240118_seamap_trawl_csv/GLFREC.csv')
BGSREC = read.csv('20240118_seamap_trawl_csv/BGSREC.csv')
ENVREC = read.csv('20240118_seamap_trawl_csv/ENVREC.csv')
STAREC = read.csv(file = "20240118_seamap_trawl_csv/STAREC.csv",
                  stringsAsFactors = F, header = T, quote = "", fill = T) #updated to read-in all the lines of the text file
CRUISES = read.csv('20190329_SEAMAP_csv/CRUISES.csv')


#cleanup for STAREC ---------
names(STAREC) <- names(starec_rev)
rm(starec_rev)

#clean quotes off of data
STAREC$COMSTAT <- NULL #subset out the 'COMSTAT' column; it causes an error in the clean.quotes function - kr
...
#exclude rows with NA values - kr added 2024-01-18
Fish_Wt_1 <- Fish_Wt_1[!is.na(Fish_Wt_1$Measurement_Type),]
...
j <- fread("filesfor_SEAMAP_QueriesRcode/jelly_inshore_20m_NULL_depth_4km_join.txt", stringsAsFactors = F, header = T)
...
j <- fread("filesfor_SEAMAP_QueriesRcode/jelly_shelf_200m_NULL_depth_4km_join.txt", stringsAsFactors = F, header = T)
...
#identifying the species belonging to either the major group or aggregrated identified above
if(use.major.grp=='yes'){
  save(x = result, file = paste0(major.grp, "_SEAMAP",Sys.Date(),".Rdata"), row.names = F)

} else if(use.aggregrated.group == 'yes'){
  save(x = result, file = paste0(aggregated.grp, "_SEAMAP",Sys.Date(),".Rdata"), row.names = F)
